...
function function_c4504c49( spawn_loc, var_75f9ae2d )
{
    ...
    return { #prop_name:prop_name, #pos_offset:pos_offset, #angle_offset:angle_offset, #rattle_sound:rattle_sound, #is_itemspawnentry:is_itemspawnentry, #var_b9cdfa83:var_e9f8015d, #var_c39008ae:var_f0c39cc2 };
}
...
function spawn_prop( spawn_loc, var_75f9ae2d, var_18840a20, var_1690a7c1 = 0 )
{
    ...
    if ( is_true( var_4d71cea4.is_itemspawnentry ) )
    {
...
function function_8f0687e( prop )
{
    ...
    lure_model = util::spawn_model( array::random( [ var_8d3beb7f, model2 ] ), var_8b84b3ce, ( 0, 0, 0 ) );
    
    if ( isdefined( lure_model ) )
    {
        lure_model clientfield::set( "mimic_prop_lure_fx", 1 );
...
{
    self endoncallback( &function_6a447863, #"death", #"stop_shake_tell" );
    self.var_624e969b = 1;
...
function transform_spawn( prop, &activators, activation_info )
{
    ...
    if ( isdefined( prop.hidden_entities ) )
    {
        ...
        foreach ( entity in prop.hidden_entities )
        {
            ...
            entity callback::callback( #"on_entity_revealed", { #activators:activators, #activation_info:activation_info } );
        }
    }
    else
    {
        ...
        if ( have_room )
        {
            ...
            new_mimic = spawnactor( level.var_15a5fa7f, prop.origin, prop.angles, undefined, 1 );
            
            if ( isdefined( new_mimic ) )
            {
                new_mimic hide();
                function_4540d40c( new_mimic, prop );
                function_d52f5cf7( prop, new_mimic );
                waitframe( 1 );
                
                if ( isdefined( new_mimic ) )
                {
                    new_mimic show();
                    new_mimic callback::callback( #"on_entity_revealed", { #activators:activators, #activation_info:activation_info } );
                }
            }
        }
        else
    ...
    {
        prop.lure_model clientfield::set( "mimic_prop_lure_fx", 0 );
        prop.lure_model deletedelay();
...
{
    self notify( #"mimic_prop_activated", { #activator:params.attacker } );
}
...
{
    trap_prop notify( #"mimic_prop_activated", { #activator:trigger_info.activator } );
}
...
{
    self.is_mimic = 0;
    self notify( #"mimic_prop_activated", { #activator:player_activator } );
}
...
{
    waitresult = self waittill( #"mimic_prop_activated" );
...
function function_f78a8a69( trigger, scriptmodel, restore_wait )
{
    ...
    scriptmodel val::set( #"mimic_lure_transform", "hide", 2 );
    scriptmodel notsolid();
    wait restore_wait;
    scriptmodel val::reset( #"mimic_lure_transform", "hide" );
    scriptmodel solid();
...
{
    level endon( #"game_ended" );
    vehicle val::set( #"mimic_lure_transform", "hide", 2 );
    vehicle notsolid();
    wait restore_wait;
