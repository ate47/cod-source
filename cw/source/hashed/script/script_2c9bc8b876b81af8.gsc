...
/#
    ...
    function function_58bee038( active_ai, spawn_ai, max_ai, var_93847faa, var_b9ce72a0 )
    {
        if ( getdvarint( #"ai_debugspawnmanager", 0 ) )
        {
            ...
            for ( i = 0; i < level.var_2db12862.size ; i++ )
            {
                ...
                text += "<dev string:xba>" + level.var_2db12862[ i ].var_5ee53e3.size + "<dev string:xce>" + level.var_2db12862[ i ].sm_active_count_min + "<dev string:xd4>" + level.var_2db12862[ i ].sm_active_count_max + "<dev string:xd9>";
                text += "<dev string:xde>" + level.var_2db12862[ i ].allspawners.size;
                
                if ( isdefined( level.var_2db12862[ i ].var_ed3f1bbb ) )
                {
                    text += "<dev string:xee>" + level.var_2db12862[ i ].var_ed3f1bbb + "<dev string:x100>" + level.var_2db12862[ i ].sm_group_size_min + "<dev string:xd4>" + level.var_2db12862[ i ].sm_group_size_max + "<dev string:xd9>";
                }
    ...
    function function_77f8229b()
    {
        ...
        if ( var_3f9688af || var_a1951b12 )
        {
            ...
            {
                case 0:
                    if ( self.var_7600cd86 + add > self.sm_active_count_max )
                    {
                        self.sm_active_count_max = self.var_7600cd86 + add;
                    }
                    
                    if ( self.var_7600cd86 + add < self.sm_active_count_min )
                    {
                        if ( self.var_7600cd86 + add > 0 )
                        {
                            self.sm_active_count_min = self.var_7600cd86 + add;
                        }
                    }
                ...
                case 4:
                    ...
                    if ( self.sm_group_size_max + add > self.var_7600cd86 )
                    {
                ...
                case 5:
                    ...
                    if ( self.var_82a78b41 + add < self.sm_spawner_count_min )
                    {
                        if ( self.var_82a78b41 + add > 0 )
                        {
                            self.sm_spawner_count_min = self.var_82a78b41 + add;
                        }
                    }
                    
                    if ( self.var_82a78b41 + add > self.sm_spawner_count_max )
                    {
                        self.sm_spawner_count_max = self.var_82a78b41 + add;
                    }
        ...
        if ( isdefined( self ) )
        {
            level.var_3ad36502 settext( "<dev string:x55a>" + self.sm_active_count_min );
            level.var_af390e0d settext( "<dev string:x573>" + self.sm_active_count_max );
            level.var_d8499d3d settext( "<dev string:x58c>" + self.sm_group_size_min );
            level.var_ae00cf1d settext( "<dev string:x5a4>" + self.sm_group_size_max );
            
            if ( isdefined( self.var_82a78b41 ) )
            {
                level.var_92f8da9a settext( "<dev string:x5bc>" + self.var_82a78b41 );
                level.var_254556d9 settext( "<dev string:x5d2>" + self.sm_spawner_count_min );
                level.var_6c6a96f9 settext( "<dev string:x5ec>" + self.sm_spawner_count_max );
            }
        }
    }
    ...
    function function_2973e587()
    {
        ...
        {
            self.var_7600cd86 = randomintrange( self.sm_active_count_min, self.sm_active_count_max + 1 );
        }
        
        self.spawners = self function_bf4a387a();
        assert( self.count >= self.count_min );
        assert( self.count <= self.count_max );
        assert( self.var_7600cd86 >= self.sm_active_count_min );
        assert( self.var_7600cd86 <= self.sm_active_count_max );
        assert( self.sm_group_size_max <= self.var_7600cd86 );
        assert( self.sm_group_size_min <= self.var_7600cd86 );
    }
