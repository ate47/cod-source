...
function function_2089690e()
{
    ...
    self show();
    spawn_anim_alias = isdefined( self.var_c9b11cb3 ) ? self.var_c9b11cb3 : self.spawn_anim_alias;
    spawn_anim = isdefined( self.var_d32d749d ) ? self.var_d32d749d : self.spawn_anim;
    
    if ( !isdefined( spawn_anim ) && isdefined( spawn_anim_alias ) )
    {
        spawn_anim = self animmappingsearch( spawn_anim_alias );
    }
    
    self.spawn_anim_alias = spawn_anim_alias;
        ...
        if ( params.einflictor.classname === "script_vehicle" && params.smeansofdeath == "MOD_CRUSH" )
        {
            ...
            if ( isdefined( n_damage ) )
            {
                if ( isalive( self ) && !is_true( self.var_490b950e ) )
                {
                    ...
                    if ( self.aitype === #"spawner_bo5_mechz_sr" || self.aitype === #"spawner_bo5_raz_sr" || self.aitype === #"spawner_zm_steiner" || self.aitype === #"hash_21f3d5d40d72e08d" )
                    {
...
{
    should_break = 0;
    is_interactable = dynent.script_noteworthy == #"hash_4d1fb8524fdfd254";
    
    if ( is_interactable || dynent.destructible )
    {
        bundle = isdefined( dynent.bundle ) ? dynent.bundle : zm_utility::function_b42da08a( dynent );
...
function function_9d3ad056( str_aitype, v_origin, v_angles, str_name, var_c427317f = 1, var_1028910 = #"hash_5d96e8e3ed203968" )
{
    ...
    n_percent = min( 0.8, level.star_rating * 0.1 + 0.5 );
...
// Size: 0x222
function function_e4791424( origin, var_79d43d2f, height, outer_radius, inner_radius = 0, return_offsets = 0, use_throttle = 1, b_expensive = 1 )
{
    level endon( #"game_ended" );
    ...
    for ( i = 0; i < var_79d43d2f ; i++ )
    {
        ...
        while ( !isdefined( final_point ) && var_8b63a38e < 5 )
        {
            ...
            final_point = function_6b273d22( origin, point, outer_radius, inner_radius, !b_expensive );
...
function function_5ba8b9d4( entity, var_21bdf069, dynent, var_f3fec032, dynentstate, is_interactable )
{
    if ( is_interactable && dynentstate != 0 || is_true( var_f3fec032.dynentstates[ dynentstate ].var_b7401b42 ) || !isdefined( dynent.var_34e210ee ) )
    {
        return false;
    }
...
function function_dce54c8d( player, radius )
{
    ...
    foreach ( dynent in var_e86a4d9 )
    {
        ...
        if ( is_interactable || dynent.destructible )
        {
            ...
            {
                should_break = function_5ba8b9d4( ai, sqr( isdefined( ai.var_c50d964 ) ? ai.var_c50d964 : ai getpathfindingradius() ), dynent, bundle, state, is_interactable );
