...
function function_39ee3b21( eventstruct, a_spawns )
{
    if ( !is_true( self.var_83523415 ) && isplayer( eventstruct.activator ) && level flag::get( #"hash_44074059e3987765" ) )
    {
        ...
        foreach ( spawn in a_spawns )
        {
            ...
            {
                if ( isdefined( spawn.spawn_anim_alias ) && spawn.spawn_anim_alias == "anim_spawn_from_fireplace" )
                {
                    ai = spawnactor( #"spawner_bo5_zombie_sr_charred", spawn.origin, spawn.angles, undefined, 1, 1 );
                }
                else
                {
                ...
                if ( isdefined( ai ) )
                {
                    if ( isdefined( spawn.spawn_anim_alias ) && spawn.spawn_anim_alias != " " && spawn.spawn_anim_alias != "" )
                    {
                        ai.var_c9b11cb3 = spawn.spawn_anim_alias;
                    }
                    else
                    {
