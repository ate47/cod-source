...
function private function_cee5c029( localclientnum, *oldval, newval, *bnewent, *binitialsnap, *fieldname, *bwastimejump )
{
    if ( bwastimejump != 0 )
    {
        ...
        if ( isdefined( data ) )
        {
            function_c79ecd60( fieldname, data.text0, data.var_d86d7aad, data.icon0, data.text1, data.var_66b29739, data.icon1, data.text2, data.var_9c7902e5, data.icon2, data.text3, data.var_cab95f65 );
        }
    }
}
    ...
    switch ( notification_id )
    {
        ...
        case 11:
            if ( isdefined( level.var_58624fd1 ) && isdefined( level.var_7dfc60c ) )
            {
                ...
                data.icon2 = function_30f7f6f8( level.var_7dfc60c );
