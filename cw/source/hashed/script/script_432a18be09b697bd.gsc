...
{
    level endon( #"hash_2d0433bbc2675311" );
    scene::add_scene_func( #"aib_t9_cust_zm_silver_easter_egg_helping_hand_flail", &function_d521aae, "flail" );
    var_f6ed35c4 = getent( "hh_door_5", "targetname" );
    ...
    level thread scene::play( #"aib_t9_cust_zm_silver_easter_egg_helping_hand_flail" );
    level waittill( #"hand_back" );
    wait 0.5;
...
{
    level endon( #"hash_2d0433bbc2675311", #"flail_anim_ended" );
    ent = a_ents[ #"claw" ];
...
function function_76f53c53()
{
    ...
    if ( !player laststand::player_is_in_laststand() && player.sessionstate !== "spectator" )
    {
        ...
        if ( item.itementry.rarity === "Legendary" )
        {
            ...
            {
                point = function_4ba8fde( #"resource_item_harvesting_zm" );
...
function function_b3dff8ff()
{
    ...
    for ( var_6a3f365 = 2; var_6a3f365 > 0 ; var_6a3f365-- )
    {
        ...
        var_b528ceef thread scene::play( #"aib_t9_cust_zm_silver_easter_egg_distant_monster", "Walk", var_b528ceef );
        level waittill( #"scene_distant_monster_walk_shutdown" );
        var_b528ceef.origin = var_b528ceef gettagorigin( "tag_player" );
    }
    
    level scene::delete_scene_spawned_ents( #"aib_t9_cust_zm_silver_easter_egg_distant_monster" );
        ...
        if ( destination.targetname === #"hash_86dbad917999c92" )
        {
            foreach ( location in destination.locations )
            {
                ...
                if ( isdefined( var_30cea8e0 ) )
                {
                    ...
                    var_60470a92 = item_supply_drop::function_9771c7db( s_portal.origin, #"supply_stash_zm_silver_dance_party" );
...
function function_1c448218( a_ents )
{
    ...
    while ( true )
    {
        level waittill( #"fwd_anim_played" );
        
        if ( self.script_noteworthy === "move_forward" )
        {
            if ( isdefined( a_ents[ #"z_single_dance" ] ) )
            {
                ent = a_ents[ #"z_single_dance" ];
...
function function_8e8d08cd()
{
    ...
    level.var_a04cebc2.trigger.var_cc1fb2d0 = #"supply_stash_zm_silver_dance_party";
