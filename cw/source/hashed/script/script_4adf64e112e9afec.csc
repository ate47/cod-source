...
function function_f5b1f2e0( localclientnum, *oldval, newval, *bnewent, *binitialsnap, *fieldname, *bwastimejump )
{
    if ( bwastimejump )
    {
        if ( !isdefined( self.health_bar ) )
        {
            ...
            self.health_bar = function_deb84496( fieldname, self getentitynumber() );
            ...
            if ( self.health_bar luielem_entity_bar::is_open( fieldname ) )
            {
                self.health_bar luielem_entity_bar::set_offset( fieldname, 0, 0, 90 );
                self.health_bar luielem_entity_bar::function_a9793a65( fieldname, 0.5 );
...
{
    if ( !self.health_bar luielem_entity_bar::is_open( localclientnum ) )
    {
        return;
    }
    ...
    self.health_bar luielem_entity_bar::function_ecacbaa5( localclientnum, self.health_percent );
    self.health_bar luielem_entity_bar::set_state( localclientnum, hash( state ) );
}
