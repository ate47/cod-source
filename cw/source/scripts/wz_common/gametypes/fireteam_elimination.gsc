...
function private function_9b927bb4( var_698dbb78 )
{
    ...
    if ( inradiation )
    {
        ...
        foreach ( var_9bbce2cd in level.var_ac7be286 )
        {
            ...
            if ( isdefined( target ) )
            {
                playertotarget = target - insertionplayer.origin;
                playertotarget = ( playertotarget[ 0 ], playertotarget[ 1 ], 0 );
                a = vectordot( playertotarget, playertotarget );
                b = 2 * vectordot( var_8675da3, playertotarget );
                c = vectordot( var_8675da3, var_8675da3 ) - radius * radius;
                d = sqrt( b * b - 4 * a * c );
                t = ( d - b ) / 2 * a;
                var_14ff2ad5 = t * playertotarget;
            }
            else
            {
...
function private function_a3e78cba()
{
    ...
    if ( level.var_8166e83a >= 0 )
    {
        ...
        {
            if ( exfilstruct.script_objective_set == level.var_8166e83a )
            {
    ...
    level.var_5d5301ba = level.exfilstruct.script_objective_set;
...
function function_93ef48b( vip, helicopter )
{
    ...
    vip setlowready( 1 );
    helicopter notify( #"vip_in_helicopter" );
}
...
function function_2d5cfefd()
{
    ...
    level.var_4afb8f5a[ #"ticket_redeploy_item_t9" ] = #"armor_shard_item_t9";
}
...
function function_7545eaf0()
{
    ...
    {
        if ( bomb.script_objective_set === level.var_5d5301ba )
        {
