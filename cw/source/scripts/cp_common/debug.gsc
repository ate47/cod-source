...
/#
    ...
    function debugdvars()
    {
        ...
        if ( getdvarstring( #"debug_lookangle" ) == "<dev string:x38>" )
        {
            setdvar( #"debug_lookangle", "<dev string:x73>" );
        }
        ...
        if ( getdvarstring( #"debug_vehicletag" ) == "<dev string:x38>" )
        {
            setdvar( #"debug_vehicletag", "<dev string:x38>" );
        }
        ...
        if ( getdvarstring( #"debug_badpath" ) == "<dev string:x38>" )
        {
            setdvar( #"debug_badpath", "<dev string:x73>" );
        }
        ...
        if ( getdvarstring( #"debug_deathents" ) == "<dev string:x99>" )
        {
            setdvar( #"debug_deathents", "<dev string:x73>" );
        }
        ...
        if ( getdvarstring( #"animsound_save" ) == "<dev string:x38>" )
        {
            setdvar( #"animsound_save", "<dev string:x38>" );
        }
        ...
        noanimscripts = getdvarstring( #"hash_3e63b96becc87615" ) == "<dev string:x99>";
        ...
        for ( ;; )
        {
            ...
            if ( getdvarstring( #"debug_vehicletag" ) != "<dev string:x38>" )
            {
                thread viewtag( "<dev string:xa6>", getdvarstring( #"debug_vehicletag" ) );
            }
            ...
            level.debug_badpath = getdvarstring( #"debug_badpath" ) == "<dev string:x99>";
            
            if ( !noanimscripts && getdvarstring( #"hash_3e63b96becc87615" ) == "<dev string:x99>" )
            {
                noanimscripts = 1;
            }
            
            if ( noanimscripts && getdvarstring( #"hash_3e63b96becc87615" ) == "<dev string:x73>" )
            {
                level.animation.defaultexception = &util::void;
                level.animation notify( #"new exceptions" );
                noanimscripts = 0;
            }
    ...
    function init_animsounds_for_animname( animname )
    {
        ...
        for ( i = 0; i < animes.size ; i++ )
        {
            ...
            level.animsound_aliases[ animname ][ anime ][ "<dev string:x1f8>" + anime ][ #"created_by_animsound" ] = 1;
        }
    }
    ...
    function function_90b2a0b0( x, y, msg )
    {
        ...
        hudelm.alignx = "<dev string:x1fd>";
        hudelm.aligny = "<dev string:x205>";
        hudelm.x = x;
        hudelm.y = y;
        hudelm.alpha = 1;
        hudelm.fontscale = 1;
        hudelm.label = msg;
        level.var_e8a7658[ level.var_e8a7658.size ] = hudelm;
        return hudelm;
    }
    ...
    function function_b03ecdf3()
    {
        ...
        if ( !isdefined( level.var_100db28b ) )
        {
            ...
            for ( i = 0; i < level.animsound_hudlimit ; i++ )
            {
                ...
                hudelm.alpha = 1;
                hudelm.fontscale = 1;
                hudelm.label = "<dev string:x38>";
                level.var_100db28b[ level.var_100db28b.size ] = hudelm;
                hudelm = newdebughudelem();
                hudelm.alignx = "<dev string:x2c6>";
                hudelm.aligny = "<dev string:x205>";
                hudelm.x = -10;
                hudelm.y = 200 + i * 10;
                hudelm.alpha = 1;
                hudelm.fontscale = 1;
                hudelm.label = "<dev string:x38>";
                level.var_eef171c[ level.var_eef171c.size ] = hudelm;
                hudelm = newdebughudelem();
                hudelm.alignx = "<dev string:x2c6>";
                hudelm.aligny = "<dev string:x205>";
                hudelm.x = 210;
                ...
                hudelm.fontscale = 1;
                hudelm.label = "<dev string:x38>";
                level.var_c22fdb8f[ level.var_c22fdb8f.size ] = hudelm;
            }
    ...
    function function_21619947()
    {
        ...
        for ( i = 0; i < level.animsound_hudlimit ; i++ )
        {
            hudelm = level.var_100db28b[ i ];
            hudelm.label = "<dev string:x38>";
            hudelm.color = ( 1, 1, 1 );
            hudelm = level.var_eef171c[ i ];
            hudelm.label = "<dev string:x38>";
            hudelm.color = ( 1, 1, 1 );
            hudelm = level.var_c22fdb8f[ i ];
            hudelm.label = "<dev string:x38>";
            hudelm.color = ( 1, 1, 1 );
        }
        ...
        level.var_77ea7dca.label = "<dev string:x222>" + animsounds[ level.animsound_selected ].anime;
        level.var_100db28b[ level.animsound_selected ].color = ( 1, 1, 0 );
        level.var_eef171c[ level.animsound_selected ].color = ( 1, 1, 0 );
        level.var_c22fdb8f[ level.animsound_selected ].color = ( 1, 1, 0 );
        time = gettime();
        ...
        for ( i = 0; i < keys.size ; i++ )
        {
            ...
            hudelm = level.var_100db28b[ key ];
            soundalias = get_alias_from_stored( animsound );
            hudelm.label = key + 1 + "<dev string:x315>" + animsound.notetrack;
            hudelm = level.var_eef171c[ key ];
            hudelm.label = int( ( time - animsound.end_time - 60000 ) * 0.001 );
            
            if ( isdefined( soundalias ) )
            {
                hudelm = level.var_c22fdb8f[ key ];
