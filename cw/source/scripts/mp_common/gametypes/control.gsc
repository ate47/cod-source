...
function on_player_killed( *einflictor, attacker, *idamage, *smeansofdeath, weapon, *vdir, *shitloc, *psoffsettime, *deathanimduration )
{
    ...
    {
        if ( isdefined( psoffsettime.pers ) && isdefined( psoffsettime.pers[ #"hash_22770dcff96626d2" ] ) && isdefined( psoffsettime.pers[ #"defensekills" ] ) )
        {
            ...
            {
                psoffsettime.pers[ #"defensekills" ]++;
                psoffsettime function_ef823e71( 12, psoffsettime.pers[ #"defensekills" ] );
            }
        }
...
function private on_zone_capture( sentient )
{
    ...
    if ( isplayer( sentient ) )
    {
        ...
        level thread telemetry::function_18135b72( #"callback_on_game_event", { #player:sentient, #eventtype:#"hardpoint_captured" } );
...
function private credit_player( player, string, capturetime, *capture_team, lastcaptureteam, var_a84f97bf, var_af8f6146 )
{
    ...
    if ( !is_score_boosting( string ) )
    {
        ...
        level thread telemetry::function_18135b72( #"callback_on_game_event", { #player:string, #eventtype:#"capture" } );
...
function private function_7996e36d()
{
    foreach ( player in level.players )
    {
        if ( player.team != #"spectator" )
        {
            ...
            game.var_629fe99f[ player.playername ][ #"defensekills" ] = player.pers[ #"defensekills" ];
...
function private function_f9bfc5ca( player )
{
    if ( game.var_629fe99f.size == 0 || !isdefined( game.var_629fe99f[ player.playername ] ) )
    {
        ...
        if ( !isdefined( player.pers[ #"defensekills" ] ) )
        {
            player.pers[ #"defensekills" ] = 0;
        }
    ...
    player.pers[ #"defensekills" ] = game.var_629fe99f[ player.playername ][ #"defensekills" ];
