...
function postinit()
{
    ...
    callback::add_callback( #"on_mimic_player_grab_start", &function_ff1fe53a );
...
function function_29307fae( struct )
{
    ...
    foreach ( point in spawn_points )
    {
        ...
        assert( isdefined( spawn_struct.contentgroups[ #"fishing_bob" ] ), "<dev string:x56>" );
        var_c4b6fa2d = spawn_struct.contentgroups[ #"fishing_bob" ];
        var_cf8192ca = content_manager::spawn_script_model( var_c4b6fa2d[ 0 ], #"p9_zm_out_fishing_01_bobber", 0 );
        assert( isdefined( spawn_struct.contentgroups[ #"fishing_end_target" ] ), "<dev string:x87>" );
        var_df283c09 = spawn_struct.contentgroups[ #"fishing_end_target" ];
