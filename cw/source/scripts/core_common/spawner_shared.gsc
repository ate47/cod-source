...
function spawn_think( spawner )
{
    ...
    if ( isdefined( spawner.spawner_fixednode ) )
    {
        self.fixednode = spawner.spawner_fixednode;
    }
    else if ( isdefined( self.spawner_fixednode ) )
    {
        self.fixednode = self.spawner_fixednode;
    }
    
    if ( isdefined( spawner ) && isdefined( spawner.spawner_script_accuracy ) )
    {
        self.script_accuracy = spawner.spawner_script_accuracy;
    }
    else if ( isdefined( self.spawner_script_accuracy ) )
    {
        self.script_accuracy = self.spawner_script_accuracy;
    }
...
function spawn_think_action( spawner )
{
    ...
    if ( isdefined( spawner.script_disable_random_speed_scale ) )
    {
...
function private go_to_node_using_funcs( node, optional_arrived_at_node_func, require_player_dist )
{
    ...
    for ( ;; )
    {
        ...
        if ( isdefined( node.scriptbundlename_exit ) )
        {
            node scene::play( node.scriptbundlename_exit, self );
        }
...
function private function_890856aa( struct )
{
    ...
    if ( is_true( struct.script_ignore_angles ) )
    {
