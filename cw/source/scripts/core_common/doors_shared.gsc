...
class cdoor
{
    ...
    function init_door_model( e_or_str_model, s_door_instance )
    {
        ...
        if ( !isdefined( m_e_door ) )
        {
            ...
            m_e_door.dont_round = s_door_instance.dont_round;
            self function_d1e7faca( 1 );
        }
        
        if ( is_true( var_a2f96f78.script_no_open_ai ) )
        {
    ...
    function function_d1e7faca( closed )
    {
        if ( closed )
        {
            if ( is_true( m_e_door.dont_round ) )
            {
        ...
        if ( isdefined( level.var_b4749b2e ) && is_true( m_e_door.dont_round ) && m_e_door islinkedto( level.var_b4749b2e ) )
        {
            m_e_door unlink();
        }
...
function setup_door_info( s_door_bundle, s_door_instance, c_door )
{
    ...
    if ( is_true( s_door_instance.script_obstruction_malfunction ) )
    {
...
{
    if ( is_true( self.var_a2f96f78.script_locked ) )
    {
        if ( !is_true( reset ) )
        {
            [[ self ]]->lock();
    ...
    if ( is_true( self.var_a2f96f78.script_lockpick ) )
    {
        if ( !is_true( reset ) )
        {
            [[ self ]]->lock();
    ...
    if ( is_true( self.var_a2f96f78.script_no_bash ) )
    {
        self.var_d587661f = 2;
    }
    
    if ( is_true( self.var_a2f96f78.script_no_open ) )
    {
        self.var_81f24576 = 2;
    }
    
    if ( isdefined( self.var_a2f96f78.script_push_open_override ) )
    {
        if ( self.var_a2f96f78.script_push_open_override == "FORCE_OFF" )
        {
            self.var_f97eab64 = 0;
            return;
        }
        
        if ( self.var_a2f96f78.script_push_open_override == "FORCE_ON" )
        {
            self.var_f97eab64 = 1;
        }
    }
...
function private function_d2a304e( a_doors )
{
    foreach ( s_instance in a_doors )
    {
        if ( is_true( s_instance.script_lockpick ) )
        {
...
function private function_1e18148c()
{
    ...
    s_minigame.script_access_mode = "Exclusive";
    s_minigame.script_delete = 1;
    s_minigame.script_difficulty = self.script_difficulty;
    s_minigame.script_enable_on_start = 1;
    s_minigame.script_lockpick_open = self.script_lockpick_open;
    s_minigame.script_minigame_weapon = 0;
    s_minigame.script_puzzle_index = self.script_puzzle_index;
