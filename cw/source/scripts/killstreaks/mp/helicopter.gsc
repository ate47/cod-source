...
function function_d45a1f8d( einflictor, attacker, *idamage, *smeansofdeath, weapon, *vdir, *shitloc, *psoffsettime, *deathanimduration )
{
    ...
    {
        shitloc killstreak_dialog::play_pilot_dialog_on_owner( "kill", "helicopter_comlink", shitloc.killstreak_id );
...
function on_vehicle_killed( params )
{
    ...
    if ( einflictor.killstreaktype === "helicopter_comlink" || einflictor.killstreaktype === "inventory_helicopter_comlink" )
    {
        ...
        {
            einflictor killstreak_dialog::play_pilot_dialog_on_owner( "killAircraft", einflictor.killstreaktype, einflictor.killstreak_id );
            return;
        }
        
        if ( !( vehicle.vehicleclass === "boat" ) )
        {
            einflictor killstreak_dialog::play_pilot_dialog_on_owner( "killGroundVehicle", einflictor.killstreaktype, einflictor.killstreak_id );
        }
    }
}
