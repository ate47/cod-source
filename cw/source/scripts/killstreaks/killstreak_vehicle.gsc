...
function activate_vehicle( type )
{
    ...
    self thread killstreak_dialog::play_killstreak_start_dialog( type, self.team, killstreak_id );
    ...
    if ( !is_true( self.abandoned ) && isplayer( attacker ) )
    {
        ...
        if ( util::function_fbce7263( self.team, attacker.team ) )
        {
            ...
            if ( isdefined( bundle ) )
            {
                ...
                if ( !var_3906173b )
                {
                    self killstreak_dialog::play_destroyed_dialog_on_owner( bundle.kstype, self.killstreak_id );
...
function function_d4896942( bundle, killstreakname, customendon )
{
    ...
    while ( true )
    {
        ...
        self.var_7132bbb7 = 1;
        self killstreak_dialog::play_pilot_dialog_on_owner( "damageEvaded", killstreakname, self.killstreak_id );
...
function function_6650cc9c( missile, bundle, killstreakname, beep_alias )
{
    ...
    {
        self killstreak_dialog::play_pilot_dialog_on_owner( "incomingMissile", killstreakname, self.killstreak_id );
        self.var_b6286a42 = gettime() + 5000;
    }
