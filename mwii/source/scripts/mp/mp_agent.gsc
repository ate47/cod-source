...
setupweapon( primaryweaponobject )
{
...
spawnnewagent( agent_type, spawn_team, spawn_position, spawn_angles, _id_F9E7DC261E07162E )
{
...
{
    setup_func = level.agent_definition[self.agent_type]["setup_func"];
    self [[ setup_func ]]();
}
...
assign_agent_func( _id_AD662D6A990F6FCC, default_func )
{
    level.agent_funcs[self.agent_type][_id_AD662D6A990F6FCC] = default_func;
}
...
getfreeagent( agent_type )
{
    ...
    freeagent = _func_76B285B4BAE7356C();
...
default_on_damage( einflictor, eattacker, idamage, idflags, smeansofdeath, sweapon, vpoint, vdir, shitloc, timeoffset, modelindex, partname, objweapon )
{
    ...
    if ( istrue( victim.agentdamagefeedback ) )
    {
        ...
        {
            biskillstreakweapon = isdefined( objweapon ) && [[ scripts\cp_mp\utility\script_utility::getsharedfunc( "killstreak", "isKillstreakWeapon" ) ]]( objweapon.basename );

            if ( scripts\cp_mp\utility\script_utility::issharedfuncdefined( "damage", "handleDamageFeedback" ) )
                eattacker [[ scripts\cp_mp\utility\script_utility::getsharedfunc( "damage", "handleDamageFeedback" ) ]]( einflictor, eattacker, victim, idamage, smeansofdeath, objweapon, shitloc, idflags, 0, 0, biskillstreakweapon );
        }
    }
...
default_on_damage_finished( einflictor, eattacker, idamage, idflags, smeansofdeath, sweapon, vpoint, vdir, shitloc, timeoffset, stun_fraction, modelindex, partname, armorhit )
{
    prevhealth = self.health;
    objweapon = sweapon;
    self.damagedby = eattacker;
    self.damagepoint = vpoint;
    self finishagentdamage( einflictor, eattacker, idamage, idflags, smeansofdeath, objweapon, vpoint, vdir, shitloc, timeoffset, 0.0, modelindex, partname, istrue( armorhit ) );
...
default_on_killed( einflictor, eattacker, idamage, smeansofdeath, objweapon, vdir, shitloc, timeoffset, deathanimduration )
{
    ...
    _id_4C770A9A4AD7659C::callback( "on_ai_killed", params_struct );
}
...
on_humanoid_agent_killed_common( einflictor, eattacker, idamage, smeansofdeath, objweapon, vdir, shitloc, timeoffset, deathanimduration, dropweapons )
{
...
_id_89FCC8D16C4FD558( corpse, vehicle, seatid, deleteonseatenter )
{
    corpse endon( "death" );
...
_id_07687424C385DE94( vehicle, seatindex )
{
    ...
    {
        if ( _id_5539341A688C012F == seatindex )
            return seatid;
    }
}
...
do_immediate_ragdoll( agent_body )
{
    ...
    initialimpulse = 10;
    damagetype = scripts\common\utility::getdamagetype( self.damagemod );
