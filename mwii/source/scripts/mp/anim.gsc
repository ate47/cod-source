...
play_sound_at_viewheight( aliasname, notification_string, _id_A68ADBD3EEFE9282 )
{
...
do_facial_anim( dofacialanim, dodialogue, doanimation, anime, animname, dialogue )
{
    return 0;
}

do_animation( org, angles, animname, anime, anim_string, idleanim, scripted_node_ent )
{
    animation = undefined;
    ...
    else
    {
        ...
        if ( isagent( self ) )
            ...
            self animscripted( anim_string, org, angles, animation, undefined, root, goaltime );
    }

    thread scripts\common\notetrack::start_notetrack_wait( self, anim_string, anime, animname, animation );
    return getanimlength( animation );
}

mp_anim_handle_notetrack( scr_notetrack, guy, dialogue_array, tag_owner )
{
    scripts\common\notetrack::anim_handle_notetrack( scr_notetrack, guy, dialogue_array, tag_owner );
...
anim_player_solo( player, player_rig, anime, tag, _id_9E8A16D47A03007A, animname_override )
{
    self endon( "death" );
    players[0] = player;
    ...
    anim_player( players, player_rigs, anime, tag, _id_9E8A16D47A03007A, animname_override );
}
...
anim_player( players, player_rigs, anime, tag, _id_9E8A16D47A03007A, animname_override )
{
    ...
    anim_player_internal( players, player_rigs, anime, tag, _id_9E8A16D47A03007A, animname_override );
}
...
anim_player_internal( players, player_rigs, anime, tag, _id_9E8A16D47A03007A, animname_override )
{
    ...
    if ( isdefined( player_rigs ) )
        scripts\common\anim::anim_single( player_rigs, anime, tag, _id_9E8A16D47A03007A, animname_override );
}
