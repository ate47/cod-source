...
bot_get_ambush_trap_item( _id_0D5160E708615340, _id_0D5163E7086159D9, _id_0D5162E7086157A6 )
{
    ...
    foreach ( purpose in _id_DBF63E06B8CA889A )
    {
        foreach ( item in _id_68939DB9F7B6D3C4 )
        {
            if ( scripts\mp\bots\bots_util::bot_item_matches_purpose( purpose, item ) )
            {
...
bot_set_ambush_trap( trap_item, ambush_entrances, ambush_node, ambush_yaw, trap_node )
{
    ...
    if ( !isdefined( trap_node ) && isdefined( ambush_entrances ) && ambush_entrances.size > 0 )
    {
        ...
        {
            _id_65277EFB57BFEF86 = scripts\engine\utility::random( choose_set );
            ...
            trap_node = self botnodepick( _id_94E232E9AE9B3161, min( _id_94E232E9AE9B3161.size, 3 ), "node_trap", ambush_node, _id_65277EFB57BFEF86 );
        }
    }
    ...
    if ( isdefined( trap_node ) )
    {
        ...
        if ( trap_item["purpose"] == "trap_directional" && isdefined( _id_65277EFB57BFEF86 ) )
        {
            _id_F5798034F8D40700 = vectortoangles( _id_65277EFB57BFEF86.origin - trap_node.origin );
        ...
        if ( goal_succeeded )
        {
            ...
            if ( result == "goal" )
            {
                ...
                if ( !isdefined( self.enemy ) || !self botcanseeentity( self.enemy ) )
                {
                    ...
                    timetowait = scripts\engine\utility::ter_op( isdefined( trap_item["purpose"] ) && trap_item["purpose"] == "tacticalinsertion", 6.0, 3.0 );
                    scripts\engine\utility::waittill_any_timeout_2( timetowait, "missile_fire", "grenade_fire" );
                    wait 0.05;
...
bot_protect_zone( nodes, defend_point, uniqueid, optional_params )
{
    if ( !isdefined( optional_params ) || !isdefined( optional_params["min_goal_time"] ) )
        optional_params["min_goal_time"] = 12;
...
bot_defend_requires_center( defense_type )
{
...
bot_defend_think( defendcenter, defendradius, defense_type, optional_params )
{
    ...
    if ( !isdefined( self.bot_defend_player_guarding ) && bot_defend_requires_center( defense_type ) )
    {
        ...
        if ( !isdefined( nearest_node ) )
        {
            ...
            {
                _id_594F44744FB80561 = vectornormalize( nodes[i].origin - defend_center );
                trace_start = defend_center + _id_594F44744FB80561 * 15;
    ...
    _id_339F8BA30BFE82BC = level.bot_find_defend_node_func[defense_type];
    ...
    for (;;)
    {
        ...
        _id_6462F213110D0C30 = min_goal_time;
        _id_9F09525E6F9A8906 = max_goal_time;
        ...
        {
            if ( _id_78A174EAB45C6C23 && scripts\mp\bots\bots_util::bot_is_protecting() && !isplayer( defendcenter ) && isdefined( self.defend_entrance_index ) )
            {
                trap_item = bot_get_ambush_trap_item( "trap_directional", "trap", "c4" );

                if ( isdefined( trap_item ) )
                {
                    entrances = scripts\mp\bots\bots_util::bot_get_entrances_for_stance_and_index( undefined, self.defend_entrance_index );
            ...
            if ( self bothasscriptgoal() )
            {
                ...
                if ( scripts\mp\bots\bots_util::bot_vectors_are_equal( current_goal, previous_goal ) )
...
calculate_defend_stance( _id_81A934C1F7D7E976, _id_97DF6EE9F2FE6188 )
{
    ...
    if ( _id_81A934C1F7D7E976 )
    {
        ...
        if ( choice < _id_B42989B458B54982 )
            stance = "crouch";
        else if ( choice < _id_B42989B458B54982 + _id_8E7073E75873765A )
            stance = "prone";
...
defense_cautious_approach()
{
    ...
    _id_51CA426712B96DF5 = radius_around_point * radius_around_point;
    _id_B3464EEEBEF71679 = getnodesinradius( self.bot_defending_center, radius_around_point, 0, 500 );
    ...
    for ( i = 0; i < _id_B784196CF6D9C696.size; i++ )
    {
        new_location = setup_investigate_location( _id_B784196CF6D9C696[i] );
        self.locations_to_investigate = scripts\engine\utility::array_add( self.locations_to_investigate, new_location );
    }
    ...
    foreach ( location in killer_locations )
    {
        if ( distancesquared( location, self.bot_defending_center ) < _id_51CA426712B96DF5 )
        {
            ...
            {
                new_location = setup_investigate_location( _id_BA8A43C132FA515A, location );
                self.locations_to_investigate = scripts\engine\utility::array_add( self.locations_to_investigate, new_location );
            }
        }
    }
    ...
    if ( isdefined( self.defend_entrance_index ) )
    {
        ...
        for ( i = 0; i < _id_77EECA99C75956F8.size; i++ )
        {
            new_location = setup_investigate_location( _id_77EECA99C75956F8[i] );
            self.locations_to_investigate = scripts\engine\utility::array_add( self.locations_to_investigate, new_location );
        }
    }
    ...
    for ( _id_F51C48051FFCA680 = 1; _id_F51C48051FFCA680 < _id_55D2448DD14FBB1E.size - 2; _id_F51C48051FFCA680++ )
    {
        ...
        for ( i = 0; i < _id_906D6B682A625FDC.size; i++ )
        {
            if ( !scripts\engine\utility::within_fov( self.origin, self getplayerangles(), _id_906D6B682A625FDC[i].origin, 0 ) )
                continue;
            ...
            for ( j = 0; j < self.locations_to_investigate.size; j++ )
            {
                ...
                if ( nodesvisible( location.node, _id_906D6B682A625FDC[i], 1 ) )
                {
                    _id_8594AEB2834C5D66 = scripts\engine\utility::array_add( _id_8594AEB2834C5D66, _id_906D6B682A625FDC[i] );
        ...
        hide_node = self botnodepick( _id_8594AEB2834C5D66, 1 + _id_8594AEB2834C5D66.size * 0.15, "node_hide" );
        ...
        if ( isdefined( hide_node ) )
        {
            ...
            for ( i = 0; i < self.locations_to_investigate.size; i++ )
            {
                if ( nodesvisible( self.locations_to_investigate[i].node, hide_node, 1 ) )
                {
                    if ( distance2dsquared( self.locations_to_investigate[i].origin, hide_node.origin ) > 3600 )
                        _id_724C1907987840A4 = scripts\engine\utility::array_add( _id_724C1907987840A4, self.locations_to_investigate[i] );
                }
            }
            ...
            if ( result == "goal" )
            {
                for ( i = 0; i < _id_724C1907987840A4.size; i++ )
                {
                    if ( distance2dsquared( self.origin, _id_724C1907987840A4[i].origin ) < 1600 )
                        continue;

                    for ( _id_EEB18C09526831F4 = 0; _id_724C1907987840A4[i].frames_visible < 18 && _id_EEB18C09526831F4 < 3.6; _id_EEB18C09526831F4 = _id_EEB18C09526831F4 + 0.25 )
                    {
                        self botlookatpoint( _id_724C1907987840A4[i].origin + ( 0, 0, self getplayerviewheight() ), 0.25, "script_search" );
                        wait 0.25;
                    }
                }
            }
...
monitor_cautious_approach_dangerous_locations()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( _id_F6CBDE8CC73449C9 ) )
        {
            ...
            for ( i = 0; i < self.locations_to_investigate.size; i++ )
            {
                if ( nodesvisible( _id_F6CBDE8CC73449C9, self.locations_to_investigate[i].node, 1 ) )
                {
                    node_within_fov = scripts\engine\utility::within_fov( self.origin, self getplayerangles(), self.locations_to_investigate[i].origin, bot_fov );
                    _id_C9F1DF8D1FD67FDA = !node_within_fov || self.locations_to_investigate[i].frames_visible < 17;

                    if ( _id_C9F1DF8D1FD67FDA && distancesquared( self.origin, self.locations_to_investigate[i].origin ) < _id_F3EDCEFBABC5A8F0 )
                    {
                        node_within_fov = 1;
...
protect_watch_allies()
{
    ...
    minimap_radius = 1050;
    _id_F5B7C9D0D13192E4 = minimap_radius * minimap_radius;
    ...
    {
        cur_time = gettime();
        _id_A2FDE156132CAD23 = bot_get_teammates_in_radius( self.bot_defending_center, radius_around_point );

        foreach ( teammate in _id_A2FDE156132CAD23 )
        {
            _id_96A7043372F6F04E = teammate.entity_number;

            if ( !isdefined( _id_96A7043372F6F04E ) )
                _id_96A7043372F6F04E = teammate getentitynumber();

            if ( !isdefined( _id_0CF1DC37D165E2F3[_id_96A7043372F6F04E] ) )
                _id_0CF1DC37D165E2F3[_id_96A7043372F6F04E] = cur_time - 1;
            ...
            {
                if ( cur_time - teammate.last_investigation_time > 10000 && cur_time > _id_0CF1DC37D165E2F3[_id_96A7043372F6F04E] )
                {
                    if ( isdefined( teammate.lastattacker ) && isdefined( teammate.lastattacker.team ) && teammate.lastattacker.team == scripts\engine\utility::get_enemy_team( self.team ) )
                    {
                        if ( distancesquared( teammate.body.origin, self.origin ) < _id_F5B7C9D0D13192E4 )
                        {
                            self botgetimperfectenemyinfo( teammate.lastattacker, teammate.body.origin );
                            _id_56681CCFF326541B = getclosestnodeinsight( teammate.body.origin );
                        ...
                        _id_0CF1DC37D165E2F3[_id_96A7043372F6F04E] = cur_time + 10000;
                    }
                }
            }
        }
...
defense_watch_entrances_at_goal()
{
    ...
    if ( scripts\mp\bots\bots_util::bot_is_capturing() )
    {
        ...
        {
            foreach ( entrance in _id_3C31225BF8CAA5EA )
            {
                if ( nodesvisible( _id_170E87A3A1E8AF42, entrance, 1 ) )
                    _id_77EECA99C75956F8 = scripts\engine\utility::array_add( _id_77EECA99C75956F8, entrance );
            }
        }
...
bot_monitor_watch_entrances_at_goal()
{
    ...
    for (;;)
    {
        ...
        foreach ( node in self.watch_nodes )
        {
            if ( _id_9BC4F8E26F7A0AC4 )
            {
                gametype_scalar = self [[ _id_16DA7E3AFE815065 ]]( node );
...
bot_monitor_watch_entrances_bodyguard()
{
    ...
    for (;;)
    {
        ...
        foreach ( node in self.watch_nodes )
        {
            ...
            _id_0A1CD126B61EABD2 = vectordot( _id_FE3D6FB608E9EED8, player_to_node );
...
{
    _id_405DAA810191CBA9 = getnodezone( entrance );
    _id_0F2ED0B797104FA5 = vectornormalize( entrance.origin - self.origin );
...
prioritize_watch_nodes_toward_enemies( scalar )
{
    ...
    for ( z = 0; z < level.zonecount; z++ )
    {
        ...
        {
            _id_E5DCBEEC9029AE9F = getnodezone( nodes_testing[i] );
            ...
            {
                _id_A295C00559B01807 = vectornormalize( nodes_testing[i].origin - self.origin );
...
entrance_watched_by_player( player, entrance )
{
    ...
    _id_0A1CD126B61EABD2 = vectordot( player_forward, player_to_node );
...
bot_get_teammates_currently_defending_zone( zoneid )
{
    ...
    {
        if ( !isai( teammate ) || teammate scripts\mp\bots\bots_util::bot_is_defending() && teammate.bot_defending_zone_id == zoneid )
            _id_33DE4B46D339A890 = scripts\engine\utility::array_add( _id_33DE4B46D339A890, teammate );
    }
...
bot_get_teammates_currently_defending_point( point, radius_around_point )
{
    ...
    foreach ( teammate in teammates_in_radius )
    {
        if ( !isai( teammate ) || teammate scripts\mp\bots\bots_util::bot_is_defending_point( point ) )
            _id_33DE4B46D339A890 = scripts\engine\utility::array_add( _id_33DE4B46D339A890, teammate );
    }
...
bot_get_teammates_in_radius( point, radius_around_point )
{
    ...
    for ( i = 0; i < level.participants.size; i++ )
    {
        ...
        if ( other_player != self && isdefined( other_player.team ) && other_player.team == self.team && scripts\mp\utility\entity::isteamparticipant( other_player ) )
        {
            if ( distancesquared( point, other_player.origin ) < _id_51CA426712B96DF5 )
                teammates_in_radius = scripts\engine\utility::array_add( teammates_in_radius, other_player );
        }
    }
...
defend_get_ally_bots_at_zone_for_stance( stance )
{
    ...
    foreach ( other_player in level.participants )
    {
        ...
        if ( other_player.team == self.team && other_player != self && isai( other_player ) && other_player scripts\mp\bots\bots_util::bot_is_defending() && isdefined( other_player.cur_defend_stance ) && other_player.cur_defend_stance == stance )
        {
            if ( other_player.bot_defending_type == self.bot_defending_type && scripts\mp\bots\bots_util::bot_is_defending_point( other_player.bot_defending_center ) )
                _id_A59D5599D9E0EE09 = scripts\engine\utility::array_add( _id_A59D5599D9E0EE09, other_player );
        }
    }
        ...
        if ( self botgetscriptgoaltype() != "none" )
        {
            ...
            if ( _id_3519B2425797C239 > 100 )
            {
                ...
                if ( distancesquared( _id_8FCB33318BC2A8B2, self.bot_defend_player_guarding.origin ) > _id_578D6AFD11258520 * _id_578D6AFD11258520 )
                {
                    ...
                    _id_3B619B88092C2AC0 = vectornormalize( script_goal - self.bot_defend_player_guarding.origin );
                    normalized_velocity = vectornormalize( _id_20D851954EAE02C4 );

                    if ( vectordot( _id_3B619B88092C2AC0, normalized_velocity ) < 0.1 )
                    {
                        self notify( "defend_force_node_recalculation" );
                        wait 0.25;
                    }
...
find_defend_node_capture()
{
    ...
    node = scripts\mp\bots\bots_util::bot_find_node_to_capture_point( scripts\mp\bots\bots_util::defend_valid_center(), self.bot_defending_radius, entrance_point );

    if ( isdefined( node ) )
    {
        if ( isdefined( entrance_point ) )
        {
            _id_D70B2C2C937442F6 = vectornormalize( entrance_point - node.origin );
    ...
    else if ( isdefined( entrance_point ) )
...
{
    entrance_point = bot_defend_get_random_entrance_point_for_current_area();
    node = scripts\mp\bots\bots_util::bot_find_node_to_capture_zone( self.bot_defending_nodes, entrance_point );
...
find_defend_node_protect()
{
    ...
    if ( isdefined( node ) )
    {
        node_to_center = vectornormalize( scripts\mp\bots\bots_util::defend_valid_center() - node.origin );
...
bot_handle_no_valid_defense_node( _id_B76B361D985E9EFE, _id_1DC58F0A2B0494CD )
{
    ...
    else
    {
        ...
        {
            angle_dir = vectornormalize( _id_B76B361D985E9EFE - self.cur_defend_point_override );
        ...
        {
            angle_dir = vectornormalize( self.cur_defend_point_override - _id_1DC58F0A2B0494CD );
            self.cur_defend_angle_override = vectortoangles( angle_dir );
        }
    }
}
    ...
    for ( i = 0; i < level.participants.size; i++ )
    {
        ...
        if ( isdefined( other_player.team ) && other_player.team == self.team && other_player != self )
        {
            if ( isai( other_player ) )
            {
                ...
                if ( _id_02AD2D07831A9ACD && other_player bothasscriptgoal() )
                {
...
bot_capture_zone_get_furthest_distance()
{
    ...
    if ( isdefined( self.bot_defending_nodes ) )
    {
        foreach ( node in self.bot_defending_nodes )
        {
            ...
            furthest_dist = max( _id_4FD3127FD90FD6E0, furthest_dist );
        }
    }
...
bot_think_tactical_goals()
{
    ...
    for (;;)
    {
        if ( self.tactical_goals.size > 0 && !scripts\mp\bots\bots_util::bot_is_remote_or_linked() )
        {
            ...
            self.tactical_goals = scripts\engine\utility::array_remove( self.tactical_goals, new_goal );
        }
...
bot_new_tactical_goal( type, goal_position, priority, extra_params )
{
    ...
    {
        if ( new_goal.priority > self.tactical_goals[_id_B5F8BF1C8EB1626E].priority )
            break;
    }

    for ( i = self.tactical_goals.size - 1; i >= _id_B5F8BF1C8EB1626E; i-- )
        self.tactical_goals[i + 1] = self.tactical_goals[i];

    self.tactical_goals[_id_B5F8BF1C8EB1626E] = new_goal;
}
...
bot_melee_tactical_insertion_check()
{
    ...
    if ( !isdefined( self.last_melee_ti_check ) || now - self.last_melee_ti_check > 1000 )
    {
        ...
        _id_C00C20554CB14554 = botzonenearestcount( myzone, self.team, 1, "enemy_predict", ">", 0 );
        ...
        place_node = self botnodepick( _id_C49A42FA6B3F0E0D, _id_C49A42FA6B3F0E0D.size * 0.15, "node_hide" );
